...
glob.sync(`src/views/${customDiretory}/**/*.js`).forEach(path => {
  ...
  pages[name] = {
    ...
    template: pagesConfig[name] && pagesConfig[name].template ?
      pagesConfig[name].template : pathArray.join('/'),
    filename: `views/${diretories.join('/')}.html`,
    title: pagesConfig[name] && pagesConfig[name].title ?
      pagesConfig[name].title : target + ' Page',
    chunks: ['chunk-vendors', 'chunk-common', name],
    // 别名设置
    from: `/${diretories.join('/')}`,
    to: `/views/${diretories.join('/')}.html`,
    // 默认会插入框架的aepmod.js
    hasAepMod: pagesConfig[name] && typeof pagesConfig[name].hasAepMod === 'boolean' ?
      pagesConfig[name].hasAepMod : true,
